...
### Global Variables
expName = sys.argv[1] #experiment name 
k = sys.argv[2] #probability threshold
...
def save_methyl_prob_plot(scores,motif,expName,k):
	"""
	Input:  scores list or numpy array containing unlogged scores (0-1)
			experiment name for the plot title (str)
	...
	plt.title(f'{expName} dSMF')
	
	plt.savefig(f'{expName}_{k}_{motif}_methyl_distribution.pdf')

	plt.close()

	return 0
	
def save_agg_freq_plot(score_list_pos,motif,expName,k):
	"""
	...
	plt.tight_layout()
	plt.savefig(f'{expName}_{k}_{motif}_agg_freq.pdf')
	plt.close()
...
def main():
	...
	for motif in motifs:
		for strand in strands:
			print(f'Retrieving scores from {expName}: {motif} strand {strand}...')
			score_list.append(np.array(get_score_list_per_motif(f'./{expName}.{motif}_1/per_read_modified_base_calls.db',strand,motif),dtype=np.float64))
	...
	print(f'dSMF...')
	save_methyl_prob_plot(np.exp(log_sc[:,1]),'CpG_GpC',expName,k)
	print(f'CpG...')
	save_methyl_prob_plot(np.exp(score_list_HCG[:,1]),'CpG',expName,k)
	print(f'GpC...')
	save_methyl_prob_plot(np.exp(score_list_GCH[:,1]),'GpC',expName,k)

	print(f'Building Aggregate Methylation Frequency Plot...')
	print(f'dSMF...')
	save_agg_freq_plot(log_sc,'CpG_GpC',expName,k)
	print(f'CpG...')
	save_agg_freq_plot(score_list_HCG,'CpG',expName,k)
	print(f'GpC...')
	save_agg_freq_plot(score_list_GCH,'GpC',expName,k)
